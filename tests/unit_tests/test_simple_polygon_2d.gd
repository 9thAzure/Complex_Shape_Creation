...
func test_get_shape_vertices__various_vertices_counts__matches_output_size(p = use_parameters([2, 5, 8, 32, 100])):
	var shape : PackedVector2Array

	shape = SimplePolygon2D.get_shape_vertices(p)

	assert_eq(shape.size(), p, "Size of returned array.")
