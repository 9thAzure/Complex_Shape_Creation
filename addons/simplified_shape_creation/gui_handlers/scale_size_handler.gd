...
func get_max_size() -> float:
	var max_size := -0.0
	for size in _shape.sizes: max_size = maxf(max_size, size)
	if _shape.ring_ratio < 0:
		max_size = lerpf(max_size, 0, _shape.ring_ratio)
	return max_size
...
func _update_properties() -> void:
	...
	for i in _old_sizes.size():
		_shape.sizes[i] = _old_sizes[i] * scale
...
func _mouse_released() -> void:
	...
	_undo_redo_manager.add_undo_property(_shape, &"sizes", _old_sizes)

	if not is_equal_approx(_old_rotation, _shape.offset_rotation):
		_undo_redo_manager.add_do_method(_shape, &"offset_rotation", _shape.offset_rotation)
